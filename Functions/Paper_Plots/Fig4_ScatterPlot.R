...
scatter_data = array(dim = c(length(DATA_past1000$CAVES$entity_info$entity_id[mask_mean]),16))
...
for(ii in 1:length(DATA_past1000$CAVES$entity_info$entity_id[mask_mean])){
  ...
  scatter_data[ii,2] = DATA_past1000$CAVES$entity_info$site_id[mask_mean][ii]
  ...
  scatter_data[ii,3] = as.numeric(mean(c(mean(data_yearly_a$ISOT, na.rm = T)- mean(data_rec$d18O_dw_eq_a, na.rm = T),
                                         mean(data_yearly_b$ISOT, na.rm = T)- mean(data_rec$d18O_dw_eq_b, na.rm = T),
                                         mean(data_yearly_c$ISOT, na.rm = T)- mean(data_rec$d18O_dw_eq_c, na.rm = T)), na.rm = T))
  scatter_data[ii,4] = as.numeric(mean(c(mean(data_rec$ISOT_a, na.rm = T)- mean(data_rec$d18O_dw_eq_a, na.rm = T),
                                         mean(data_rec$ISOT_b, na.rm = T)- mean(data_rec$d18O_dw_eq_b, na.rm = T),
                                         mean(data_rec$ISOT_c, na.rm = T)- mean(data_rec$d18O_dw_eq_c, na.rm = T)), na.rm = T))
  ...
  scatter_data[ii,15] = as.numeric(mean(data_rec$d18O_measurement, na.rm = T))
...
scatter_data$diff_full = as.numeric(levels(scatter_data$diff_full))[scatter_data$diff_full]
scatter_data$diff_down = as.numeric(levels(scatter_data$diff_down))[scatter_data$diff_down]
...
print(paste0("Calcite: ", sum(mask_mean_calcite), ", aragonite: ", sum(mask_mean_aragonite)))

cexaxis = 1.5

##diff_down
cairo_pdf(width=8,height=10,file= "Paper_Plots/Fig4_ScatterMean_diff_down.pdf")
...
mtext(text = TeX("$\\delta^{18}O_{calcite}$ (‰)"),side = 2,line = 2)
...
abline(lm.out, lwd = 4, col = "#B2182B")
mtext(text = "latitude  (°N)",side = 1,line = 2)
mtext(text = TeX("$\\delta^{18}O - \\delta^{18}O_{dw}$ (‰)"),side = 2,line = 2)
mtext(text = paste0("r²= ", round(summary(lm.out)$r.squared, digits = No.digits)), cex = 1, col = "#B2182B", adj = 1, side = 1, line = -1)
...
#geology
plot(c(0,4), c(-10,10), type = "n", xlab = "", ylab = "", yaxt = 'n', xaxt = 'n', panel.first =)
abline(h=0)
...
#prec
plot(scatter_data$mean_prec[mask_mean_calcite], scatter_data$diff_down[mask_mean_calcite], 
...
rm(scatter_data, mask_mean_aragonite, mask_mean_calcite, data_seasonal_a, data_seasonal_b, data_seasonal_c, data_yearly_a, data_yearly_b, data_yearly_c, lm.out,
   cexaxis, entity, ii, No.digits, data_rec)
